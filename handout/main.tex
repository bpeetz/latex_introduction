...
\LaTeX-Dateien sind in zwei Bereiche unterteilt: die preamble und der Inhaltsteil. In ersterer werden die Art des Dokuments ({\color{blue}\verb|\documentclass{}|}), verschiedene Metadaten (u.A. {\color{blue}\verb|\author|}), und erste Layout-Informationen festgelegt.
...
Auch Quellenangaben und generieren einer Bibliographie ist möglich. Zudem können externe .tex-Dateien mit {\color{blue}\verb|\imput{}|} ins Dokument eingebunden werden. 
...
\underline{Den Pfad zum Bild angeben:} {\color{blue}\verb|\graphicspath{{Downloads/beispiel.png}}|}\\
Das Bild \texttt{beispiel.png} liegt hier in einem Ordner "Downloads".\\ Zusätzlich kann die Größe des Bildes durch {\color{blue} \verb|"width="|} angegeben werden, in diesem Fall ist das Bild 50\% so groß, wie der Text, auch kann man sich auf die Texthöhe mit {\color{blue}"height=\verb|\textheight"|} beziehen. \\
...
Man kann
\begin{enumerate}
    \item mit {\color{blue}itemize} ungeordnete Listen (mit Aufzählungspunkt) und 
    \item mit {\color{blue}enumerate} geordnete Listen
\end{enumerate}
darstellen.

\section*{Mathematik}

Um auch etwas Mathematik hier einfließen zu lassen, kann man mathematische Formeln darstellen. Wenn man das im Satz tun möchte, passiert das am leichtesten durch das Einklammern mit den {\color{blue}\verb|$…$|}-Zeichen.\\ Möchte man eine mathematische Formel als einzigen Inhalt einer Zeile haben, nutzt man {\color{blue}\verb|\[...\]|} zum Einklammern.\\
\underline{Beispiel:}\\
...
Durch {\color{blue}\verb|\textit{}|} können auch ganze Sätze kursiv geschrieben werden. Für kursiv benutzt man {\color{blue}textit}, für fett {\color{blue}textbf} und bei unterstrichenen Sätzen {\color{blue}underline}, dabei kann ein ganzer Satz von den Klammern umschlossen werden. \\
\underline{Beispiel:}\\
...
Die Bibliothek wird dann als neue Datei mit dem Namen Name.bib erstellt und hinzugefügt durch {\color{blue}\verb|\addbibresource{sample.bib}|}.\\
